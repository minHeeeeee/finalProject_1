...
import userpage.bean.OrderCancelDTO;
...
public class UserPageServiceImpl implements UserPageService {
	...
	public void temppwd(HttpServletResponse response, Map<String, String> map) throws Exception {
		...
		System.out.println(userDTO);
			...
			map.put("user_pwd", passwordEncoder.encode(pw));
			map.put("rank_num", "4");
			//회원 등급 변경
	...
	public void temppwd2(Map<String, String> map) {
		...
		map.put("user_pwd", passwordEncoder.encode(numStr));
		map.put("rank_num", "4");
		userPageDAO.updatePwd(map);
    	try {
	...
	public List<ListDTO> getMainProduct() {
		return userPageDAO.getMainProduct();
	}
	@Override
	...
	public List<OrderPageItemDTO> getGoodsInfo(List<OrderPageItemDTO> orders) {
		...
		for(OrderPageItemDTO ord : orders) {
			...
			OrderPageItemDTO goodsInfo = userPageDAO.getGoodsInfo(ord.getProduct_sort_num());

			goodsInfo.setProduct_count(ord.getProduct_count());	
	...
	public int order(OrderDTO ord) {
		...
		//카트지운다
		for(CartListDTO c : userPageDAO.cartList(ord.getUser_id())) {
			userPageDAO.deleteCart(c.getCart_number());
		}
		//오더만든다 시퀀스 내보낸다
		return userPageDAO.order(ord);
	}
	...
	/* 주문, 주문상품 객체 */
		/*회원*/
			User_InfoDTO member = userPageDAO.getMemberInfo(dto.getUser_id());
		/*주문상품*/
			List<OrderItemDTO> ords = userPageDAO.getOrderItemInfo(dto.getOrder_id());
			for(OrderItemDTO ord : ords) {
				ord.initSaleTotal();
			}
		/* 주문 */
			OrderDTO orw = userPageDAO.getOrder(dto.getOrder_id());
			//orw.setOrders(ords);
			
			//orw.getOrderPriceInfo();
			
	/* 주문상품 취소 DB */
			userPageDAO.orderCancle(dto.getOrder_id());
...
//			member.setMoney(calMoney);
			...
			/* 포인트 */
			int calPoint = member.getMileage();
			//calPoint = calPoint + orw.getUseMileage();
			member.setMileage(calPoint);
			
				/* DB적용 */
			userPageDAO.deductMoney(member);
				
			/* 재고 */
//			for(OrderItemDTO ord : orw.getOrders()) {
//				Product_SortDTO productSort = userPageDAO.getProductsInfo(ord.getProduct_sort_num());
//				productSort.setProduct_stock(productSort.getProduct_stock() + ord.getProduct_count());
//				userPageDAO.deductStock(productSort);
//			}		
	...
	public void orderdetail(OrderItemDTO oid) {
		userPageDAO.orderdetail(oid);
	}
	@Override
	public List<OrderDTO> getOrderHistory(String user_id) {
		return userPageDAO.getOrderHistory(user_id);
	}
}
