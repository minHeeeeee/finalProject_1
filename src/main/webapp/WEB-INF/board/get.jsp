<!DOCTYPE html>
<%@page import="userpage.bean.User_InfoDTO"%>
...
<div id="help_main">
	...
	<c:if test="${map.board_category_num eq '1'}">
		...
		%>
	<c:if test="<%= user_InfoDTO != null %>">
			<input id="rank_num" type="hidden" name='rank_num' value="<%= user_InfoDTO.getRank_num()%>">
			<input id="loginUser_id" type="hidden" name='user_id' value="<%= user_InfoDTO.getUser_id()%>">
		</c:if>
	</div>
</div> <!-- end #help_main -->
